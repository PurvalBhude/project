...
class CustomCNN(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            # Input channel is 1 (for stacked features), height is N_MELS + N_MFCC (128 + 40 = 168)
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(2), # Output: (128, 21, 50)
        )
        self.classifier = nn.Sequential(
            # Adjust input features for Flatten based on new (1, 168, 400) input size
            # (128 channels * 21 height * 50 width)
            nn.Flatten(),
            nn.Linear(128 * 21 * 50, 256), # Updated linear layer input size
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(256, 2)
        )
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Resize both to Fixed Time Dimension (MAX_TIME_STEPS)
            ...
            # Stack Features (Concatenate along Feature Dimension - vertical stack)
            combined_features = np.vstack((mel_spec, mfcc)) # Shape: (N_MELS + N_MFCC, MAX_TIME_STEPS) -> (168, 400)

            # Convert to PyTorch tensor and add channel dimension (1 channel)
            combined_features = torch.from_numpy(combined_features).float().unsqueeze(0) # Shape: (1, 168, 400)

        except Exception as e:
            print(f"Error processing {file_path}: {e}")
            # Return a dummy tensor and label for error handling
...
def main(args):
    print("code started")
    run = wandb.init(project="deepfake-audio-mfcc-conversion", config=vars(args))
    config = wandb.config
    ...
    for epoch in range(config.epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            # Skip invalid samples if __getitem__ returns -1 for label
            if (labels == -1).any():
